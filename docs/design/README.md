...
#### 3. 临时组长团队业绩提成
- **计算方式**: 全额计算
- **计算依据**: 组内成员达标业绩累加（>=达标线，默认3000元），**不含临时组长自己**
- **管理提成**: 无
- **数量限制**: 仅1位
- **阶梯规则**:
  - 0-10000元: 0%
...
#### 4. 分主管团队业绩提成
- **计算方式**: 全额计算
- **计算依据**: 所管理各组所有人达标业绩累加（>=达标线，默认0元），**包含分主管自己**
- **管理提成**: 无
- **阶梯规则**:
  - 0-50000元: 0%
  - 50000元以上: 10%
...
```python
class Person:
    managed_groups: List[str] = field(default_factory=list)  # 分主管管理的组别列表
```

**身份规则对比**：

| 身份 | 数量限制 | 含自己业绩 | 管理提成 | 达标线 |
|------|----------|------------|----------|--------|
| 组长 | 多位 | ✓ | ✓ | 3000元 |
| 临时组长 | 仅1位 | ✗ | ✗ | 3000元 |
| 分主管 | 多位 | ✓ | ✗ | 0元 |
...
class Config:
    ...
    # 新增字段
    temp_leader_commission: CommissionRule          # 临时组长阶梯
    temp_leader_eligible_threshold: float = 3000.0  # 临时组长达标线
    branch_manager_commission: CommissionRule       # 分主管阶梯
    branch_manager_eligible_threshold: float = 0.0  # 分主管达标线
```
...
```python
sales_champion_threshold: float = 20000.0  # 销冠业绩阈值
sales_champion_bonus: float = 500.0        # 销冠奖金金额
```

**CommissionResult新增**：

```python
sales_champion_bonus: float = 0.0  # 销冠奖金
commission_rate: float = 0.0       # 提成点数（比例×100，4位小数）
```
...
**新增列**：
- 提成点数（commission_rate，4位小数）
- 销冠奖金（sales_champion_bonus）